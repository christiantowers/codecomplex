		...
		if(count($fields) > 0)
		{
			...
			foreach($fields as $field)
			{
				...
				if($is_filter)
				{
					...
					$name = 'name="' . get_value($field, 'table') . '[' . get_value($field, 'table_column') . ']"';
				}
	...
	*/
	public function build_array_field_db_from_object($obj_field = null, $table = '')
	{
		...
		if(!is_null($obj_field))
		{
			...
			$this->CI->load->model('acme/libraries/form_model');
			...
			$field['order'] = get_value($field_data, 'ordinal_position') * 10;
			$field['validations'] = (get_value($field_data, 'is_nullable') != 'YES') ? 'required;' : '';
			
			switch(get_value($field_data, 'data_type'))
			{
				case 'varchar':
					$field['type'] = 'text';
			...
			if(get_value($field_data, 'constraint_type') == 'FOREIGN KEY')
			{
				...
				$field['description'] = '';
				$field['options_sql'] = 'SELECT ' . $obj_field->name . ', ' . $obj_field->name . ' FROM ' . get_value($field_data, 'referenced_table_name');
			}
		}
