...
class Base_Module_Model extends CI_Model {
	...
	*/
	public function select($primary_key_value = 0)
	{
		...
		// Chave primaria
		$sql .= (is_integer_($primary_key_value)) ? " WHERE " . $this->primary_key . " = '" . $primary_key_value . "'" : '';
	...
	*/
	...
	{
		$sql = "SELECT * FROM acm_module_menu WHERE id_module = $id_module AND dtt_inative IS NULL ORDER BY `order`";
		$data = $this->db->query($sql);
		$data = $data->result_array();
		return (isset($data)) ? $data : array();
	}
	...
	*/
	...
	{
		$sql = "SELECT * FROM acm_module_action WHERE id_module = $id_module AND dtt_inative IS NULL ORDER BY `order`";
		$data = $this->db->query($sql);
		$data = $data->result_array();
		return (isset($data)) ? $data : array();
	}
