...
<script type="text/javascript" language="javascript">
	...
	// Função chaveia a troca de arquivo, quando este existe
	function change_file_page_url()
	{
		if(window.confirm("<?php echo lang('ATENÇÃO!')?>\n\n<?php echo lang('Alterar o arquivo atual fará com que ele seja descartado. Deseja prosseguir ?')?>"))
		{
			$('#box_actual_file').hide();
			$('#box_load_file_new').show();
		}
	}
	
	// Função cancela a troca de arquivo
	function cancel_file_upload()
	{
		$('#file_path_content').val('');
		$('#box_actual_file').show();
		$('#box_load_file_new').hide();
	}
</script>
<div>
	...
	<div id="module_header">
		...
		<div id="module_menus" class="inline top">
			<div class="inline top module_menu_item" title="<?php echo lang('Editar Página URL')?>" style="margin:-7px 0 0 -15px">
				<h4 class="inline top font_shadow_gray"> > <?php echo lang('Editar Página URL') ?></h4>
	...
	<div style="margin-top:50px">
		<form id="form_default" name="form_default" action="<?php echo URL_ROOT ?>/cms_page_url/form_update_custom_process" method="post" enctype="multipart/form-data">
			<input type="hidden" id="id_page_url" name="id_page_url" value="<?php echo get_value($form_data, 'id_page_url'); ?>" />
			...
			<div id="form_line">
				...
				<div style="width:100%">
					<div id="box_load_file" style="<?php echo (get_value($form_data, 'method_show') == 'load_file') ? 'display:block' : 'display:none'; ?>">
						<?php if(get_value($form_data, 'file_path_content') != '') { ?>
						<div id="box_actual_file" style="background-color:#f5f5f5;padding:10px 20px 30px 20px;border-bottom:2px solid #ccc;border-top:2px solid #ccc;margin-top:15px;padding-top:5px;">
							<img src="<?php echo URL_IMG ?>/icon_arrow_balloon_up.png" style="float:left;margin:-16px 0 0 30px;position:absolute" />
							<div>
								<img src="<?php echo URL_IMG ?>/icon_website_file.png" style="float:left;margin:5px 10px 0 -5px;" />
								<div class="inline top font_11" style="margin-top:10px;">
									<span class="bold"><?php echo basename(get_value($form_data, 'file_path_content')); ?></span>
									<div style="margin:10px 0 0 1px;">
										<a href="javascript:void(0)" onclick="download_file('<?php echo URL_ROOT ?>/cms_page_url/download_page_url_file/<?php echo get_value($form_data, 'id_page_url')?>')"><?php echo lang('Download')?></a>&nbsp;&nbsp;&nbsp;
										<a href="javascript:void(0)" onclick="change_file_page_url()"><?php echo lang('Alterar Arquivo')?></a>
									</div>
								</div>
							</div>
						</div>
						<?php } ?>
						<div id="box_load_file_new" style="background-color:#f5f5f5;padding:10px 20px 30px 20px;border-bottom:2px solid #ccc;border-top:2px solid #ccc;margin-top:15px;padding-top:5px;<?php echo (get_value($form_data, 'file_path_content') == '') ? 'display:block' : 'display:none'; ?>">
							<img src="<?php echo URL_IMG ?>/icon_arrow_balloon_up.png" style="float:left;margin:-16px 0 0 30px;position:absolute" />
							...
							<h6 style="margin:10px 0"><a href="javascript:void(0)" onclick="cancel_file_upload();"><?php echo lang('Cancelar envio de arquivo') ?></a></h6>
							<input type="hidden" name="file_path_content" id="file_path_content" value="<?php echo eval_replace(get_value($form_data, 'file_path_content')); ?>" />
						</div>
					</div>
			...
			<div id="form_line">
				<div class="form_label font_11 bold" style="width:150px"><?php echo lang('Página Ativa ?')?>*</div>
				<div class="form_field" style="max-width:700px">
					<input type="radio" name="dtt_inative" id="dtt_inative" value="" style="margin:7px 5px 0 0" <?php echo (get_value($form_data, 'dtt_inative') == '') ? 'checked="checked"' : ''?> /><h6 class="font_success inline top"><?php echo lang('Sim') ?></h6>
					<br />
					<input type="radio" name="dtt_inative" id="dtt_inative" value="CURRENT_TIMESTAMP" style="margin:7px 5px 0 0" <?php echo (get_value($form_data, 'dtt_inative') != '') ? 'checked="checked"' : ''?> /><h6 class="font_error inline top"><?php echo lang('Não') ?></h6>
					<div class="font_11 comment" style="margin-top:5px;"><?php echo lang('Ativar a página ou não fará com que ela seja exibida ou não quando for requisitada.') ?></div>
				</div>
			</div>
