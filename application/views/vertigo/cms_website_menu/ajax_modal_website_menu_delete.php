<?php
	function custom_menu_tree($menus = array(), $data = array(), $children_forever = false) 
	{
		...
		{
			if(get_value($menu, 'id_menu') == get_value($data, 'id_website_menu') || $children_forever == true)
			{
				...
				if($count_menu_children > 0)
				{
					...
					$html .= '<div class="dd-handle dd3-handle">Drag</div><div class="dd3-content">' . lang(get_value($menu, 'rotule'));
						$html .= '<div id="box_controls" style="float:right;display:none;">';
						$html .= '<img src="' . URL_IMG . '/icon_update.png" title="Editar Menu" onclick="iframe_modal(\'' . lang('Editar Menu') . '\', \'' . URL_ROOT . '/cms_website_menu/ajax_modal_website_menu_update/' . get_value($menu, 'id_menu') . '\', \'' . URL_IMG . '/icon_update.png\', 700, 600)" style="cursor:pointer;margin-right:10px" />';
						$html .= '<img src="' . URL_IMG . '/icon_delete.png" title="Deletar Menu" onclick="iframe_modal(\'' . lang('Deletar Menu') . '\', \'' . URL_ROOT . '/cms_website_menu/ajax_modal_website_menu_delete/' . get_value($menu, 'id_menu') . '\', \'' . URL_IMG . '/icon_delte.png\', 700, 600)" style="cursor:pointer" />';
						$html .= '</div>';
					$html .= '</div>';
...
<body>
	<div id="modal_content">
		...
		<!-- FORMULARIO -->
		<form id="form_default" name="form_default" action="<?php echo URL_ROOT ?>/cms_website_menu/ajax_modal_website_menu_delete_process" method="post">
			<input type="hidden" name="id_menu" id="id_menu" value="<?php echo get_value($data, 'id_website_menu') ?>" />
			<br />
			...
			<div id="box_group_view">
				<div class="odd">
					...
					<div id="field_view"><?php echo get_value($data, 'id_website_menu') ?></div>
				</div>
				<div>
					<div id="label_view"><?php echo lang('Rótulo do Menu') ?></div>
					<div id="field_view"><?php echo lang(get_value($data, 'rotule')) ?></div>
				</div>
				<div class="odd">
